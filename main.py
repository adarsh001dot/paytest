"""
===========================================
🤖 COMPLETE TELEGRAM BOT - ALL FEATURES WORKING + PAYMENT GATEWAY
===========================================
Developer: @VIP_X_OFFICIAL
Version: 9.0 (WITH AUTO PAYMENT)
Features: 100+ Features Working + UPI Auto Payment
...
POINT_PACKAGES = {
    ...
    "200": {"points": 200, "price": 100, "emoji": "💫", "popular": False},
    "500": {"points": 500, "price": 250, "emoji": "✨", "popular": True},
    "1000": {"points": 1000, "price": 500, "emoji": "⭐", "popular": True},
    "2000": {"points": 2000, "price": 1000, "emoji": "🌟", "popular": False},
    "5000": {"points": 5000, "price": 2500, "emoji": "💎", "popular": False},
}
...
# Conversation States (keeping all existing ones + new payment states)
(
    CONTACT_ADMIN,
    GENERATE_CODE,
    ADD_POINTS,
    ...
    SET_DAILY_BONUS,
    ADMIN_REPLY,
    PAYMENT_PROCESS,  # New payment state
) = range(30)
...
try:
    ...
    payment_logs_col = db['payment_logs']  # New collection for payment logs
    ...
    orders_col.create_index('gateway_order_id', unique=True, sparse=True)  # New index
    ...
    if not settings_col.find_one({'key': 'bot_settings'}):
        settings_col.insert_one({
            ...
            'daily_bonus': 1,
            'welcome_bonus': 2,
            'payment_gateway': PAYMENT_GATEWAY,  # Save payment config
    ...
    print(f"📊 Database: tg_to_num_test")
    print(f"📁 Collections:")
    ...
    print(f"   ├─ payment_logs: {payment_logs_col.count_documents({})}")
    print(f"   └─ referrals: {referral_col.count_documents({})}")
    print("="*50)
    print("✅ PAYMENT GATEWAY CONFIGURED:")
    print(f"   💳 API: {PAYMENT_GATEWAY['api_url']}")
    print(f"   🔗 Webhook: {PAYMENT_GATEWAY['webhook_url']}")
    print("="*50)
...
LANG = {
    'hi': {
        ...
        'pay_now': "💳 अभी पेमेंट करें",
        'payment_success': "✅ पेमेंट सफल!\n\n💰 राशि: ₹{}\n🎯 पॉइंट्स: {} जोड़े गए\n💎 नया बैलेंस: {}\n\nधन्यवाद!",
        'payment_failed': "❌ पेमेंट असफल! कृपया फिर से प्रयास करें।",
        'payment_pending': "⏳ पेमेंट लंबित है... वेरिफिकेशन के बाद पॉइंट्स मिलेंगे।",
    ...
    'en': {
        ...
        'payment_pending': "⏳ Payment pending... Points will be added after verification.",
...
async def create_payment_order(user_id, amount, points, order_id):
    """Create payment order using gateway API"""
    try:
        # Prepare payload
        payload = {
            'customer_mobile': '9999999999',  # Default mobile if not available
            'user_token': PAYMENT_GATEWAY['user_token'],
            'amount': str(amount),
            'order_id': order_id,
            'redirect_url': PAYMENT_GATEWAY['redirect_url'],
            'remark1': str(user_id),  # Store user_id in remark1
            'remark2': f"points_{points}"  # Store points in remark2
        }
        
        # Get user's mobile if available (you can add mobile field to users collection if needed)
        user = users_col.find_one({'user_id': user_id})
        ...
        response = requests.post(
            f"{PAYMENT_GATEWAY['api_url']}/create-order",
            data=payload,
            timeout=30
        )
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == True and data.get('result'):
                return {
                    'success': True,
                    'gateway_order_id': data['result']['orderId'],
...
async def check_payment_status(gateway_order_id):
    ...
    try:
        ...
        response = requests.post(
            f"{PAYMENT_GATEWAY['api_url']}/check-order-status",
            data=payload,
            timeout=30
        )
...
async def process_successful_payment(user_id, amount, points, order_id, gateway_order_id, utr=None):
    ...
    new_balance = await add_points(
        user_id, 
        points, 
        f"Payment for order {order_id} (Gateway: {gateway_order_id})"
    )
...
# ==================== POINTS SYSTEM WITH AUTO PAYMENT ====================
...
async def buy_points_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, package in POINT_PACKAGES.items():
        ...
        price_display = f"₹{package['price']}"
        keyboard.append([InlineKeyboardButton(
            f"{package['emoji']} {popular}{package['points']} Points - {price_display}",
...
async def process_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Process point purchase - Now with auto payment gateway"""
    query = update.callback_query
    await query.answer()
    ...
    payment_result = await create_payment_order(user_id, amount, points, order_id)
    ...
    if payment_result['success']:
        ...
        keyboard = [
            [InlineKeyboardButton(LANG[lang]['pay_now'], url=payment_result['payment_url'])],
        ...
        await query.edit_message_text(
            LANG[lang]['payment_link'].format(amount, points),
            reply_markup=reply_markup
        )
    else:
...
async def check_payment_status_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if order['status'] == 'completed':
        await query.edit_message_text(
            "✅ Payment already completed! Points have been added to your account."
        )
        return
    ...
    if order['status'] == 'failed':
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🛒 Buy Again", callback_data="buy_points")
            ]])
        )
        return
    
    # Check status with gateway
    if 'gateway_order_id' in order:
        status_result = await check_payment_status(order['gateway_order_id'])
        
        # Check if payment successful
        if status_result.get('status') == 'SUCCESS' or status_result.get('resultInfo') == 'Transaction Success':
            # Process successful payment
            new_balance = await process_successful_payment(
                user_id,
                order['amount'],
                order['points'],
                order_id,
            ...
            if new_balance:
                ...
                success_msg = LANG[lang]['payment_success'].format(
                    order['amount'],
                    order['points'],
                ...
                await query.edit_message_text(
                    "✅ Payment already processed! Points have been added to your account."
                )
        elif status_result.get('status') == 'FAILED' or status_result.get('resultInfo') == 'Transaction Failed':
            # Update order status
            orders_col.update_one(
                {'order_id': order_id},
                {'$set': {'status': 'failed'}}
...
async def show_faq(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    faq_text = (
        ...
        f"A: Buy Points पर क्लिक करें और UPI से पेमेंट करें\n\n"
        ...
        f"Q: पेमेंट कितने समय में वेरिफाई होता है?\n"
        f"A: ऑटोमेटिक वेरिफिकेशन - तुरंत पॉइंट्स मिल जाते हैं\n\n"
...
async def admin_backup_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    backup_data = {
        ...
        'payment_logs': list(payment_logs_col.find())
    }
...
def main():
    ...
    print("="*50)
    print("✅ AUTO PAYMENT GATEWAY INTEGRATED:")
    print(f"   💳 API: {PAYMENT_GATEWAY['api_url']}")
    print(f"   🔗 Webhook: {PAYMENT_GATEWAY['webhook_url']}")
    print("="*50)
