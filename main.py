...
Version: 8.0 (FINAL) + AUTO PAYMENT GATEWAY
Features: 100+ Features Working + Auto Payment
...
# ==================== CONFIGURATION ====================
BOT_TOKEN = "8294367270:AAGrQMiILErw-1SBXQ-vJrI_-z_NtVsM1SQ"
...
PAYMENT_REDIRECT_URL = "https://your_redirect_url/"  # Change this to your webhook URL
...
async def buy_points_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, package in POINT_PACKAGES.items():
        ...
        keyboard.append([InlineKeyboardButton(
            ...
            callback_data=f"auto_buy_pkg_{package['points']}"
        )])
...
async def process_auto_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Process automatic purchase via payment gateway"""
    query = update.callback_query
    await query.answer()
    
    user_id = query.from_user.id
    lang = get_user_lang(user_id)
    points = int(query.data.split('_')[3])  # auto_buy_pkg_5 -> points=5
    amount = points * 5  # ₹5 per point
    ...
    # Generate unique order ID
    order_id = f"TXN{user_id}{int(time.time())}{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        if result.get("status") == "SUCCESS":
            ...
            gateway_order_id = result["result"]["orderId"]
            ...
            orders_col.insert_one({
                ...
                'payment_method': 'AUTO',
                'created_at': get_ist(),
                'last_checked': get_ist()
            })
            
            # Send payment link to user
            keyboard = [
                [InlineKeyboardButton("💳 CLICK TO PAY", url=payment_url)],
                [InlineKeyboardButton("✅ CHECK PAYMENT", callback_data=f"check_payment_{order_id}")],
            ...
            keyboard = [[InlineKeyboardButton("🔙 Try Again", callback_data="buy_points")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await query.edit_message_text(
                f"❌ Payment gateway error: {result.get('message', 'Unknown error')}\n\nPlease try again later.",
                reply_markup=reply_markup
            )
...
async def check_payment_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if order.get('status') == 'completed':
        await query.edit_message_text(
            f"✅ Payment already completed!\n\n"
            f"Points added: {order['points']}\n"
            f"Current balance: Check /points",
    ...
    status_data = {
        "user_token": PAYMENT_USER_TOKEN,
        "order_id": order.get('gateway_order_id', order_id)
    }
    
    try:
        response = requests.post(PAYMENT_CHECK_URL, data=status_data, timeout=30)
        result = response.json()
        
        # Update last checked
        orders_col.update_one(
            {'order_id': order_id},
            {'$set': {'last_checked': get_ist()}}
        )
        ...
        if result.get("status") == "COMPLETED":
            ...
            new_balance = await add_points(
                user_id, 
                order['points'], 
                f"Auto payment for order {order_id}",
                None
            )
            ...
            orders_col.update_one(
                ...
                {'$set': {
                    ...
                    'utr': result.get('result', {}).get('utr', 'N/A')
                }}
            )
            ...
            await query.edit_message_text(
                ...
                f"💎 New balance: {format_number(new_balance)} points\n"
                f"🧾 UTR: {result.get('result', {}).get('utr', 'N/A')}\n\n"
            ...
            await query.edit_message_text(
                ...
                f"Amount: ₹{order['amount']}\n"
                f"Points: {order['points']}\n\n"
                f"Please complete the payment and click CHECK AGAIN.\n"
                f"Payment verification takes 1-2 minutes.",
                reply_markup=reply_markup
            )
    
    except Exception as e:
        keyboard = [
            [InlineKeyboardButton("🔄 Try Again", callback_data=f"check_payment_{order_id}")],
            [InlineKeyboardButton("📞 Contact Admin", callback_data="contact_admin")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text(
            f"❌ Error checking payment: {str(e)}\n\n"
            f"Please try again or contact admin.",
            reply_markup=reply_markup
        )
...
def main():
    ...
    # AUTO PAYMENT HANDLERS - NEW
    application.add_handler(CallbackQueryHandler(process_auto_purchase, pattern="^auto_buy_pkg_\\d+$"))
    ...
    print(f"   💳 Automatic Payment Processing")
    print(f"   🔄 Auto Points Addition")
    print(f"   ✅ No Manual Verification Needed")
    print("="*50)
    print("✅ ALL FEATURES LOADED AND WORKING:")
    print("   ✓ User System")
    print("   ✓ Point System")
    print("   ✓ Purchase System (AUTO PAYMENT)")
